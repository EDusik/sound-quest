...
function getLabelsError(fieldErrors: Record<string, string>): string | undefined {
  ...
  const labelEntry = Object.entries(fieldErrors).find(([k]) =>
    k.startsWith("labels."),
  );
  return labelEntry?.[1];
}
    ...
    >
      <form onSubmit={onSubmit}>
        <div className="min-w-0 space-y-6 px-5 py-4 text-foreground">
          <div>
            ...
            <div className="mt-1.5 flex items-center gap-2">
              <input
                ...
                value={formTitle}
                onChange={(e) =>
                  onFormTitleChange(e.target.value.slice(0, TITLE_MAX))
                }
                maxLength={TITLE_MAX}
                required
          ...
          />
          {submitError && (
            <div className="rounded-lg bg-red-500/20 p-3 text-sm text-red-200" role="alert">
              {submitError}
            </div>
          )}
        </div>
