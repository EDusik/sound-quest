...
export function aiLibraryChatStorageKeyScene(
  userId: string,
  sceneId: string,
): string {
...
function normalizeSuggestionTypes(
  raw: unknown,
  ...
  for (const [k, v] of Object.entries(raw as Record<string, unknown>)) {
    const idx = Number(k);
    if (!Number.isInteger(idx) || typeof v !== "string" || !TYPE_SET.has(v))
      continue;
...
export function loadAiLibraryChatState(key: string): LoadedAiLibraryChatState | null {
  ...
  try {
    ...
    if (!Array.isArray(o.messages) || !o.messages.every(isChatMessage))
      return null;
    if (!Array.isArray(o.lastSuggestions) || !o.lastSuggestions.every(isSuggestion))
      return null;
