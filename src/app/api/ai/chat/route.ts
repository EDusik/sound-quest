...
async function enrichSuggestions(
  ...
  return Promise.all(
    suggestions.map(async (s) => {
      ...
      const isPixabay =
        s.source.toLowerCase().includes("pixabay") ||
        s.sourceUrl.includes("pixabay.com");
      const isFreesound =
        s.source.toLowerCase().includes("freesound") ||
        s.sourceUrl.includes("freesound.org");
          ...
          if (
            ...
            if (!fullText.includes("---SUGGESTIONS---")) {
              ...
              if (delimIdx === -1) {
                ...
                sendSSE(controller, encoder, "text", {
                  text: textChunk.slice(0, delimIdx),
                });
              }
            }
          }
