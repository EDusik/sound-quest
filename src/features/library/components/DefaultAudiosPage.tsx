...
import { audioItemFromDefaultCatalogItem } from "@/lib/audio/mappers/default-item-to-audio-item";
import { DEFAULT_CATALOG_PLAYER_SCENE_ID } from "@/lib/audio/mappers/default-item-to-audio-item";
...
export function DefaultAudiosPage() {
  ...
  const { data: scenes = [], isLoading: scenesLoading } = useScenesQuery(user?.uid);
  const addToSceneDisabled = Boolean(user?.uid) && (scenesLoading || scenes.length === 0);
  ...
  const { data: defaultFavorites = [], isPending: favoritesPending } =
    usePublicLibraryDefaultFavoritesQuery();
  ...
  const showNoSearchResults = hasAnyItems && filtered.length === 0 && search.trim().length > 0;

  const catalogAndFavoritesReady = !catalogPending && !isLoadError && !favoritesPending;
  ...
  const showPageLoader = !isLoadError && (catalogPending || favoritesPending || !fileMetadataReady);

  const showEmptyCatalog =
    catalogAndFavoritesReady && fileMetadataReady && catalogSuccess && !hasAnyItems;
    ...
    async (sceneIds: string[]) => {
      ...
      try {
        ...
        if (sceneIds.length === 1 && sceneTitles.length === 1) {
          toast.success(t("defaultAudiosPage.addedToScene", { sceneName: sceneTitles[0] }));
        } else if (sceneTitles.length > 0) {
          toast.success(
            t("defaultAudiosPage.addedToScenes", { sceneNames: sceneTitles.join(", ") }),
          );
        } else {
  ...
  return (
    <div className="bg-background min-h-screen">
      ...
      />
      <section className="mx-auto max-w-6xl px-4 py-6 pb-24" aria-label={t("defaultAudiosPage.title")}>
        {isLoadError ? (
            ...
            {showEmptyCatalog ? (
              ...
              <div className="grid grid-cols-1 gap-8 md:grid-cols-2 md:gap-x-6 md:gap-y-8">
                {displaySections.map(({ slug, icon: Icon, items: sectionItems }) => (
                  <section key={slug} className="min-w-0" aria-labelledby={`default-cat-${slug}`}>
                    <div className="mb-3 flex items-start gap-2.5">
                      ...
                      <div className="min-w-0 pt-0.5">
                        <h2 id={`default-cat-${slug}`} className="text-sm font-semibold text-foreground">
                      ...
                      {sectionItems.map((item) => (
                        ...
                        >
                          ...
                          <AudioRow
                            ...
                            className="shrink-0"
                            onAddToScene={user?.uid ? (a) => openAddToScene(a) : undefined}
