...
import type { AudioLibraryRow } from "@/lib/audio/mappers/audio-library-map";
import { rowToItem } from "@/lib/audio/mappers/audio-library-map";
import {
  jsonForbidden,
  jsonServerError,
  jsonUnauthorized,
...
export async function DELETE(
  ...
  const { error } = await supabase.from("audio_library").delete().eq("id", parsedParams.data.id);
