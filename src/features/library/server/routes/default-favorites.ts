...
} from "@/lib/audio/mappers/library-default-favorite-map";
import {
  jsonForbidden,
  jsonServerError,
  jsonUnauthorized,
...
export async function GET(request: NextRequest) {
  ...
  const urlById = new Map(
    (libs ?? []).map((l: { id: string; source_url: string }) => [l.id, l.source_url]),
  );
...
export async function POST(request: NextRequest) {
  ...
  if (existing) {
    return NextResponse.json({ error: "Already in default sounds" }, { status: 409 });
  }
  ...
  if (typeUpdErr) {
    ...
    return librarySupabaseErrorResponse(
      typeUpdErr,
      " POST default-favorites sync library type",
    );
  }
