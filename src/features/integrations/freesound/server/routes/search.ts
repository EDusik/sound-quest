...
export async function GET(request: NextRequest) {
  ...
  if (!query) {
    return NextResponse.json({ errorCode: "freesound.errorQueryMissing" }, { status: 400 });
  }
  ...
  const pageSize = Math.min(30, Math.max(1, parseInt(searchParams.get("pageSize") ?? "15", 10)));
  ...
  try {
    ...
    if (err instanceof DOMException && err.name === "AbortError") {
      return NextResponse.json({ errorCode: "freesound.errorTimeout" }, { status: 504 });
    }
    return NextResponse.json({ errorCode: "freesound.errorNetwork" }, { status: 502 });
  } finally {
    clearTimeout(timer);
  }

  if (!res.ok) {
    return NextResponse.json({ errorCode: "freesound.errorApi" }, { status: res.status });
  }
